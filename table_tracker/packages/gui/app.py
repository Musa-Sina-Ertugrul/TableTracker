...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.sidebar_button_1 = customtkinter.CTkButton(
            self.sidebar_frame, text="Close Connection", command=self.close_connection
        )
        ...
        self.syntax_error_label = customtkinter.CTkLabel(
            self.sidebar_frame, text="Syntax Error", anchor="w"
        )
        ...
        self.enter_db = customtkinter.CTkEntry(
            self, placeholder_text="Create or Enter DB"
        )
        ...
        self.select_page_menu = customtkinter.CTkOptionMenu(
            master=self, values=["Page 1"], command=self.change_output_page
        )
    ...
    def _connect_db(self):
        if ".db" == self.get_db_name[-3:]:
            ...
            try:
                self._main_connection: sqlite3.Connection = sqlite3.connect(
                    self.get_db_name
                )
